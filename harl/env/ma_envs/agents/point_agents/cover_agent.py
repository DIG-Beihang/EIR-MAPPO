...
class CoverAgent(Agent):
    def __init__(self, experiment):
        ...
        self.dim_a = 2
        self.target = 0
        self.n_int_point = experiment.int_points_num
        ...
        if self.obs_mode == '2d_rbf':
            ...
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + np.prod(self.dim_int_points_o) + self.dim_equ_o
        elif self.obs_mode == 'mappo_obs_mode':
            self.dim_rec_o = (self.n_agents - 1, 3 + int(self.obs_radius <= 100))
            self.dim_local_o = 3 * int(not self.torus)
            self.dim_equ_o = 4
            self.dim_int_points_o = (self.n_int_point, 3)
    ...
    def observation_space(self):
        ...
        ob_space.dim_equ_o = self.dim_equ_o
    ...
    def get_observation(self, dm, my_orientation, their_orientation, feat, vels):
        ...
        if self.obs_mode == 'fix':
        ...
        elif self.obs_mode == 'tmc_local_acc':
            if self.observation_radius > 100:
                dist_to_int_points = int_points_dists / self.obs_radius
                angle_to_int_points = [np.cos(int_points_bearings), np.sin(int_points_bearings)]
                local_obs = np.zeros(self.dim_local_o)
            dist_to_int_points = int_points_dists 
            angle_to_int_points = [np.cos(int_points_bearings), np.sin(int_points_bearings)]
            ...
            int_points_obs[:, 0] = np.where(int_points_obs[:, 0] > self.observation_radius, 0, int_points_obs[:, 0]) 
            int_points_obs[:, 1] = np.where(int_points_obs[:, 0] > self.observation_radius, 0, int_points_obs[:, 1]) 
            int_points_obs[:, 2] = np.where(int_points_obs[:, 0] > self.observation_radius, 0, int_points_obs[:, 2]) 
            ...
            tmc_obs = np.zeros(self.dim_rec_o)
            tmc_obs[:, 0] = np.concatenate([navigator_dists[0:ind], navigator_dists[ind + 1:]]) 
            tmc_obs[:, 1] = np.cos(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))
            tmc_obs[:, 2] = np.sin(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))
            
            tmc_obs[:, 0] = np.where(tmc_obs[:, 0] > self.communication_radius, 0, tmc_obs[:, 0]) 
            tmc_obs[:, 1] = np.where(tmc_obs[:, 0] > self.communication_radius, 0, tmc_obs[:, 1]) 
            tmc_obs[:, 2] = np.where(tmc_obs[:, 0] > self.communication_radius, 0, tmc_obs[:, 2])

            obs = np.hstack([tmc_obs.flatten(), int_points_obs.flatten(),local_obs.flatten(), equ_obs.flatten()])
            ...
            tmc_obs = np.zeros(self.dim_rec_o)
            tmc_obs[:, 0] = np.concatenate([navigator_dists[0:ind], navigator_dists[ind + 1:]]) / self.comm_radius
            tmc_obs[:, 1] = np.cos(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))
            tmc_obs[:, 2] = np.sin(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))
