...
class PointAgent(Agent):
    def __init__(self, experiment):
        ...
        if self.obs_mode == '2d_rbf':
            ...
            self.dim_flat_o = np.prod(self.dim_evader_o) + self.dim_local_o
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + np.prod(self.dim_evader_o) + self.dim_local_o + self.dim_equ_o
            ...
            self.dim_flat_o = np.prod(self.dim_evader_o) + self.dim_local_o
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + np.prod(self.dim_evader_o) + self.dim_local_o + self.dim_equ_o + self.dim_inv_o
        self.r_matrix = None
    ...
    def get_observation(self, dm, my_orientation, their_orientation, feat, vels):
        ...
        if self.obs_mode == 'fix':
            ...
            eghn_obs[:, 0] = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.comm_radius
            eghn_obs[:, 1] = np.cos(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))
            eghn_obs[:, 2] = np.sin(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))
            ...
            obs = np.hstack([eghn_obs.flatten(), evader_obs.flatten(), equ_obs.flatten()])
        elif self.obs_mode == 'eghn':
            dist_to_evader = evader_dists / self.obs_radius
            angle_to_evader = [np.cos(evader_bearings), np.sin(evader_bearings)]
            ...
            obs = np.hstack([inv_obs.flatten(), local_obs.flatten(), evader_obs.flatten(), equ_obs.flatten()])
        elif self.obs_mode == 'sum_obs':
