...
class ActorBufferState:
    ...
    def __init__(self, args, obs_space, act_space):
        ...
        self.obs_noise = np.zeros(
            (self.episode_length, self.n_rollout_threads, *obs_shape), dtype=np.float32)
        self.obs_noise_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, *obs_shape), dtype=np.float32)
    ...
    def insert(self, obs, obs_noise, obs_noise_log_probs, rnn_states, actions, action_log_probs, masks, active_masks=None, available_actions=None):
        self.obs[self.step + 1] = obs.copy()
        self.obs_noise[self.step] = obs_noise.copy()
        self.obs_noise_log_probs[self.step] = obs_noise_log_probs.copy()
    ...
    def feed_forward_generator_actor(self, advantages, actor_num_mini_batch=None, mini_batch_size=None):
        ...
        obs_noise = self.obs_noise.reshape(-1, *self.obs_noise.shape[2:])
        obs_noise_log_probs = self.obs_noise_log_probs.reshape(-1, *self.obs_noise_log_probs.shape[2:])
        rnn_states = self.rnn_states[:-
                                     1].reshape(-1, *self.rnn_states.shape[2:])
        ...
        for indices in sampler:
            ...
            obs_noise_batch = obs_noise[indices]
            obs_noise_log_probs_batch = obs_noise_log_probs[indices]
            ...
            if self.factor is None:
                yield obs_batch, obs_noise_batch, obs_noise_log_probs_batch, rnn_states_batch, actions_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
            else:
                factor_batch = factor[indices]
                yield obs_batch, obs_noise_batch, obs_noise_log_probs_batch, rnn_states_batch, actions_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch
