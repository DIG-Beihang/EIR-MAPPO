...
from harl.env.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv
import json
...
def make_train_env(env_name, seed, n_threads, env_args):
    ...
    if env_name == "dexhands":
        from harl.env.dexhands.dexhands_env import DexHandsEnv
        return DexHandsEnv(
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac" or env_name == "smac_traitor":
                from harl.env.smac.StarCraft2_Env import StarCraft2Env
                env = StarCraft2Env(env_args)
            elif env_name == "smacv2":
                from harl.env.smacv2.smacv2_env import SMACv2Env
                env = SMACv2Env(env_args)
            elif env_name == "mamujoco":
                from harl.env.mamujoco.multiagent_mujoco.mujoco_multi import MujocoMulti
                env = MujocoMulti(env_args=env_args)
            elif env_name == "pettingzoo_mpe":
                from harl.env.pettingzoo_mpe.pettingzoo_mpe_env import PettingZooMPEEnv
                ...
                from harl.env.gym.gym_env import GYMEnv
                env = GYMEnv(env_args)
            elif env_name == "football":
                from harl.env.football.football_env import FootballEnv
                env = FootballEnv(env_args)
            elif env_name == "toy":
                from harl.env.toy_example.toy_example import ToyExample
                env = ToyExample(env_args)
            elif env_name == "lbforaging":
                from harl.env.lbforaging import ForagingEnv
                env = ForagingEnv(env_args)
            elif env_name == "rware":
                from harl.env.rware import Warehouse
                env = Warehouse(env_args)
            elif env_name == "rendezvous":
                from harl.env.ma_envs.rendezvous_env import RENDEEnv
                env = RENDEEnv(env_args)
            elif env_name == "pursuit":
                from harl.env.ma_envs.pursuit_env import PursuitEnv
                env = PursuitEnv(env_args)
            elif env_name == "navigation":
                from harl.env.ma_envs.navigation_env import NavigationEnv
                env = NavigationEnv(env_args)
            elif env_name == "cover":
                from harl.env.ma_envs.cover_env import CoverEnv
...
def make_render_env(env_name, seed, env_args):
    ...
    if env_name == "smac" or env_name == "smac_traitor":
        ...
        from harl.env.lbforaging.environment import ForagingEnv
        env = ForagingEnv(env_args)
        env.seed(seed * 60000)
    elif env_name == "toy":
        from harl.env.toy_example.toy_example import ToyExample
        ...
        manual_delay = False
            ...
            # from harl.envs.rendezvous.rendezvous import RENDEEnv
        from harl.env.ma_envs.rendezvous_env import RENDEEnv
        env = RENDEEnv(env_args)
        env.seed(seed * 60000)
    elif env_name == "pursuit":
        ...
        from harl.envs.ma_envs.navigation_env import NavigationEnv
        env = NavigationEnv(env_args)
        env.seed(seed * 60000)
    elif env_name == "cover":
        from harl.envs.ma_envs.cover_env import CoverEnv
        env = CoverEnv(env_args)
        env.seed(seed * 60000)
    else:
        print("Can not support the " +
...
def get_num_agents(env, env_args, envs):
    ...
    if env == "smac" or env == "smac_traitor":
        from harl.env.smac.smac_maps import get_map_params
