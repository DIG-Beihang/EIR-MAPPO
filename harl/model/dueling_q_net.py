...
class DuelingQNet(nn.Module):
    def __init__(self, args, obs_space, output_dim, device=torch.device("cpu")):
        ...
        # feature extractor
        if len(obs_shape) == 3:
            self.feature_extractor = PlainCNN(obs_shape, base_hidden_sizes[0], base_activation_func)
        ...
        # dueling a
        dueling_a_sizes = [base_hidden_sizes[-1]] + list(dueling_a_hidden_sizes) + [output_dim]
