...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.obs_add_global_pos = kwargs["env_args"].get(
        ...
        self.obs_agent_adversary = kwargs["env_args"].get("obs_agent_adversary", False)
        ...
        if self.agent_obsk is not None:
            ...
            self.k_categories = [k_split[k if k < len(
                k_split) else -1].split(",") for k in range(self.agent_obsk + 1)]

            self.global_categories_label = kwargs["env_args"].get(
                "global_categories")
    ...
    def step(self, actions):
        ...
        flat_actions = np.concatenate(
            [actions[i][:self.action_space[i].low.shape[0]] for i in range(self.n_agents)])
        flat_actions = np.clip(flat_actions, -1, 1)
    ...
    def get_obs(self):
        ...
        for a in range(self.n_agents):
            ...
            if self.obs_agent_adversary:
                obs_i = np.concatenate([obs_i, [0 for _ in range(self.n_agents)]])
            obs_n.append(obs_i)
        return obs_n
    ...
    def get_state(self, team=None):
        ...
        if self.obs_agent_adversary:
            state_normed = np.concatenate([state_normed, [0 for _ in range(self.n_agents)]])
        share_obs = []
        for a in range(self.n_agents):
