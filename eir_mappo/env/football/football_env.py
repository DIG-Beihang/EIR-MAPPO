...
class FootballEnv:
    ...
    def step(self, actions):
        ...
        return self.split(obs), self.repeat(self.get_state()), rewards, self.repeat(done), [info], self.avail_actions
    ...
    def process_args(self, args):
        if "obs_agent_adversary" in args:
            self.obs_agent_adversary = args["obs_agent_adversary"]
            del args["obs_agent_adversary"]
        else:
            self.obs_agent_adversary = False
    ...
    def get_state_space(self):
        ...
        if self.obs_agent_adversary:
            total_length += self.n_agents
        return Box(low=-np.inf, high=np.inf, shape=(total_length,), dtype=self.env.observation_space.dtype)
    ...
    def get_obs_shape(self):
        ...
        if self.img:
            ...
            return [Box(low=obs_sp.low[idx].transpose(2, 0, 1), high=obs_sp.high[idx].transpose(2, 0, 1), shape=(c, w, h), dtype=obs_sp.dtype) for idx in range(self.n_agents)]
        else:
            if self.obs_agent_adversary:
                shape = (obs_sp.shape[1] + self.n_agents,)
            else:
                shape = obs_sp.shape[1:]
            return [Box(low=-np.inf, high=np.inf, shape=shape, dtype=obs_sp.dtype) for _ in range(self.n_agents)]
    ...
    def split(self, a):
        if self.obs_agent_adversary:
            return [np.concatenate([a[i], np.zeros(self.n_agents)]) for i in range(self.n_agents)]
        else:
