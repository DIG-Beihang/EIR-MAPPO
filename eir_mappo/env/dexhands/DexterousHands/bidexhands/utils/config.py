...
def load_env_cfg(args, use_rlg_config=False):
    with open(os.path.join(os.path.split(os.path.dirname(os.path.abspath(__file__)))[
            0], args.cfg_env), 'r') as f:
...
def get_args(env_args, benchmark=False, use_rlg_config=False):
    custom_parameters = [
        {"name": "--env", "type": str, "default": "dexhands",
            "help": "env to run"},
        {"name": "--exp_name", "type": str, "default": "installtest",
        ...
        {"name": "--task", "type": str, "default": env_args["task"],
        ...
        {"name": "--num_envs", "type": int, "default": env_args["n_threads"],
            "help": "Number of environments to create - override config file"},
        {"name": "--episode_length", "type": int, "default": env_args["hands_episode_length"],
