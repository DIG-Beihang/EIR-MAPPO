...
from eir_mappo.util.util import get_init_method
"""RNN modules."""
    ...
    def forward(self, x, hxs, masks):
        if x.size(0) == hxs.size(0):
            x, hxs = self.rnn(x.unsqueeze(0),
                              (hxs * masks.repeat(1, self.recurrent_N).unsqueeze(-1)).transpose(0, 1).contiguous())
            x = x.squeeze(0)
            ...
            for i in range(len(has_zeros) - 1):
                ...
                temp = (hxs * masks[start_idx].view(1, -1,
                        1).repeat(self.recurrent_N, 1, 1)).contiguous()
